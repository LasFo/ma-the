...
The only result of the computation phase is that the \code{StmState} is computed.
Hence, the core data structure is the \code{StmState} which holds all informations to process a transaction:
\begin{lstlisting}
...
IORef, the second and empty list and the third \code{()}. With a the value gained from 
\code{getGlobalId} an new TVar is created. We assume the ID is \code{1} for now. 
...
Implementation. If the TVar is present in the \code{writeSet}, the \code{currentValue} is returned.
...
After wrapping the value for type reason with \code{Just}, it is entered in the \code{expectedValue} and \code{newValue} 
...
\end{lstlisting}
In both, the first and the second field, \code{Just (val + 1)} is entered. \code{notifies} is 
...
on the initial state. This state is used in atomically to commit the changes.
