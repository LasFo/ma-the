...
This is the only function (besides \code{atomically}) that uses IO. A TVar consists of three MVars
and one ID. To ensure the uniqueness of the IDs the STM library defines \code{globalCount :: MVar Int}
and a function \code{getGlobalId :: IO Int} that takes the global count increments it, writes it back 
...
All MVar are allocated by \code{newMVar}. This means all MVars are initialized when the TVar is created.
...
Implementation. If the TVar is present in the \code{writeSet}, the associated value is returned.
The differences are in the other part of \code{readTVar}. Recall the type of \code{readTVar :: TVar a -> STM a}.
The result is a STM action. This means a function that takes a \code{StmState} and returns an \code{StmResult}. 
...
\code{buildVal} wraps
the value with \code{unsafePerformIO}. The IO action consists of two parts. First, it IO-reads the 
...
information to the \code{readSet} that are needed to validate\footnote{Notice the difference between 
\code{readSet} and \code{ReadSet}. \code{readSet} is the field of the \code{StmState} and 
\code{ReadSet} is the data type.}. These are the ID of the TVar as 
...
part of the IO action. The other information are the arguments of \code{buildVal}. After \code{buildVal}
...
After wrapping the value for type reason with \code{Just}, it is entered in the first and second 
fiel of the associated entry in the \code{writeSet}. The last field of the entry is the MVar of
...
In the end the new state and \code{()} are returned.
...
only differ in this part. The performance tests presented in \textbf{REF TO EVALUATION}
provide not a clear result on which implementation is better in general. I will present
...
writes executed by the first action. Since the \code{readSet} is a \code{IORef}, 
...
locked are stored within \code{IntMap}s. If we try to lock these TVars, we convert the \code{IntMap} to a
list and process it. Since the conversion from an \code{IntMap} constructs an ordered list, we lock always 
