...
data STMState = TST {touchedTVars :: IntMap.IntMap (IO (IO())),
                     ...
                     writeSet     :: IntMap.IntMap ((),MVar (),NodeVal),
                     notifys      :: IO(),
                     retryMVar    :: MVar ()} 
...
readTVar (TVar mv id waitQ) = STM (\state ->
                case IntMap.lookup id (writeSet state) of
                   Just (v,_,node) -> return (Success state (SV (return (unsafeCoerce v),[],[node])))
                   Nothing -> do
                     newMV <- newEmptyMVar
                     wq <- takeMVar waitQ
                     putMVar waitQ (newMV : wq)
                     let newNode = NV (newMV,Nothing,id)
                         newState = state{touchedTVars = IntMap.insert id (io mv) (touchedTVars state),
...
writeTVar (TVar mv id waitQ) (SV (act,wqs,nodes)) = STM (\state -> do
        res <- act
        newMV <- newEmptyMVar 
        let newNode = NV (newMV,Just (unsafeCoerce act),id)
            newState = state{touchedTVars = IntMap.insert id (io mv) (touchedTVars state),
                             writeSet = IntMap.insert id (unsafeCoerce res,unsafeCoerce mv,newNode)
                                                         (writeSet state),
...
newTVar val = STM (\state -> do
    ...
    wq <- newMVar []
    id <- getGlobalId
    return (Success state (TVar nmv id wq)))
...
redesign :: [(ID,())] -> IntMap.IntMap ((),MVar (), NodeVal) ->  IntMap.IntMap ((),MVar (), NodeVal)
redesign [] map = map
...
reevaluate :: [NodeVal] -> IO [(ID,())]
reevaluate [] = return []
reevaluate ((NV (_,act,id)):xs) = do
  res <- reevaluate xs
  if isNothing act 
       then return res
       else do val <- fromJust act 
               return ((id,unsafeCoerce val):res)
