...
instance Monad STM where
  (STM tr1)  >>= k = STM (\state -> do
               stmRes <- tr1 state
               case stmRes of
                 Success newState deps ma -> do
                   enter deps (retryMVar state)
                   seq ma $ return ()
                   valid <- tryTakeMVar $ retryMVar state
                   if isNothing valid 
                      then do let (STM tr2) = k (fromJust ma) 
                              tr2 newState
                      else return InValid
                 Retry newState -> return (Retry newState)
                 InValid -> return InValid)
...
data StmState = TST {touchedTVars  :: IntMap.IntMap (IO(IO())),
                     ...
                     writeSet      :: IntMap.IntMap (Maybe (),Maybe (),MVar (),Deps), 
                     notifys       :: IO (),
                     retryMVar     :: MVar (),
                     garbage       :: [OneTuple ()]} 
               ...
               let res = buildVal mv
                   newState = stmState{touchedTVars = case IntMap.lookup id (touchedTVars stmState) of
                                       ...
                                       writeSet = IntMap.insert id 
                                                      (unsafeCoerce res, Nothing, 
                                                       unsafeCoerce mv, [waitQ])
                                                      (writeSet stmState)}
...
{-# NOINLINE buildVal #-}
buildVal :: MVar a -> Maybe a
buildVal ioRef = unsafePerformIO $ {-print "IORead" >>-} readMVar ioRef >>= return . Just
              ...
              Success newState deps val -> do
                 let finState =
                      newState{touchedTVars = case IntMap.lookup id (touchedTVars newState) of
                               ...
                               writeSet = IntMap.insert id 
                                              (unsafeCoerce val, unsafeCoerce val,
                                               unsafeCoerce ioRef, deps)
                                              (writeSet newState),
    ...
    atomically' stmAction state = do
      ...
      case stmResult of
        ...
        InValid -> do
          ...
          let reState = state{retryMVar = rMVar}
          atomically' stmAction reState
        Success newState _deps res -> do --at this point waitQs are no longer important
...
--in one traverse.
write :: [(Maybe (), Maybe (),MVar(), Deps)] -> IO (IO ())
write ws = do
    (writes, locks) <- readWS ws
    locks
    return writes
  where readWS []               = return (return (),return())
        readWS ((_old,Nothing,_ref,_deps):xs) = readWS xs
        readWS ((_old,Just a,ref,_deps):xs)   = do
             (ws,locks) <- readWS xs
