...
single element buffers. If you are using this interface the underlying implementation ensures the so called \keyword{ACI(D)} properties. 
...
an interface which allows the user to process arbitrary operation on one element buffers (so called TVars). The operations can be grouped to \keyword{transactions}.
