...
\code{MVar} and returns the value. \code{putMVar} is the opposite operation. It fills the MVar with a value, if it is empty and 
...
you are wondering when I use \code{STM} and when STM. I use \code{STM} when I refer to the Haskell type constructor and STM when I refer to STM as library.}. 
...
The function \code{atomically :: STM a -> IO a} is used to execute an STM action. 
...
This is because STM ensures the \keyword{ACID} properties. The ACID properties were Introduced by \parencite{DBTrans} for
...
\end{itemize}
In the case of software transactions the \keyword{Durability} is not demanded, which is why we will refer to the ACI properties
for the rest of the thesis.

These properties explain the name \code{atomically}. The enclosed code appears to be executed instantaneously without any 
...
restarts immediately (and no TVar has changed), the transaction executes the same code, including \code{retry}. 
...
Note that it is not possible to execute \code{IO} actions within a transaction, which means that no side effects can occur. Furthermore, this means
...
The reason this works is because always the whole transaction is considered as one block for which
...
If it is present, the \code{newValue} is returned. If it is not present, a new entry is created. While \code{tvar}
is the passed TVar, \code{newValue} and \code{expectedValue} are the current value of the TVar.
...
set to the value passed to \code{writeTVar}. If it is not present, a new entry is created. The \code{tvar} is the passed TVar,
the \code{newValue} is the passed value and \code{expectedValue} is the current value of that TVar. This is the other time 
...
transaction = do
  ...
  writeTVar t2 a
\end{lstlisting}
This code would lead to the following log\footnote{The names of the variables \code{a} and \code{b} are choosen
...
part the \code{expectedValue} and the last part is the \code{newValue}. The first entry contains the information that \code{t1}
held the value \code{a} when it was first read and the value \code{b} was written to \code{t1}. \code{t2} held the value \code{b} 
...
is integrated in the log of the surrounding transaction, but the \code{newValue} fields of the inner log are ignored. Afterwards the second
...
Validation denotes the process to check if the \code{expectedValues} are equal to the current values in the TVars.
In other words, for each entry in the log the transaction compares the \code{expectedValue} in the log with the 
current value of the actual TVar. If at least one of these values does not match, the transaction is considered
\keyword{invalid}. If the validation returns invalid the transaction is instantaneously rolled back.
If all values match the the transaction is considered \keyword{valid}. If the validation
...
The transaction iterates through the log and updates the actual TVars if \code{expectedValue} and \code{newValue} differ. 
...
Then the second transaction tries to commit. Since the actual value has changed to \code{2}, it does no longer match 
...
intended behaviour after incrementing the TVar twice that holds \code{1}. This is the well known \keyword{lost update}
problem. 
...
\subsubsection{Deferred and Direct Updates} The way an STM system modifies the underlying data structures can either be \keyword{deferred}
...
\subsubsection{Synchronization} Another important property of an STM system is the way it synchronizes transactions. In order to validate
...
to use \code{atomically}, by which this action becomes a transaction. STM in Haskell provides strong isolation although it is possible 
...
\keyword{critical} for the transaction (see Section \ref{definitions} for definition). Critical means a modifications of that TVar causes the transaction to roll back.
Figure \ref{fig:criticalValue} visualizes when the TVars \code{acc1} and \code{acc2} are critical for \code{transaction1}.
When \code{readTVar acc1} is executed the TVar becomes critical and stays critical until the transaction commits.
...
execution of \code{readTVar} can reduce the time TVars are critical and by this the chance the transaction is rolled back.
\begin{figure}
\centering
...
\end{figure}
Our aim is to delay the execution of \code{readTVar} as far as possible to reduce the time 
...
The critical time is minimal if the TVars are read directly before or at the start of the commit phase.
...
Assume \code{transaction1} gets the locks first and tries to validate. 
...
\code{t2} was not modified. Hence the exact same code with same inputs and the same (relevant) environment 
...
the entry for \code{acc1} from the log and execute the first two actions of transfer instead of all actions.
...
the TVar. After \code{t1} has read the TVar and before it has commited, transaction \code{t2} writes this TVar (calls \code{writeTVar}). 
This means that \code{t1} works with an outdated value after \code{t2} commits. In the current implementation
conflicts are not detected until both, \code{t1} and \code{t2}, try to commit. If only \code{t1} commits, it does not 
know that \code{t2} has written the TVar. If only \code{t2} commits, it does not know that \code{t1} has read this TVar.
This allows both transactions to commit without rolling back either of them, if \code{t1} commits before \code{t2}.
Hence, a conflict not necessarily includes a violation of the ACI properties.
If \code{t2} commit first, the conflict is detected when \code{t1} commits and resolved by rolling back \code{t1}.
...
implementation, each time a transaction executes \code{readTVar}, the read TVar becomes critical for this transaction. Note that the
...
is a static code property. We assume that a thread is either \code{active}, \code{ready} or \code{suspended}.
If the thread is \code{suspended}, it waits for an external signal to swap to the \code{ready} state. If it
is \code{ready} the scheduler is allowed to swap this thread to \code{running} which grants the thread access to
the CPU. If the thread executes specific operation (for example the semaphore operation \code{P}) it enters 
the state \code{suspended}. Otherwise it is up to the scheduler to swap the thread to the \code{ready} state and
...
an STM action that is supposed to be executed with \code{atomically}. I use the term \code{STM} action
...
\end{lstlisting}
\code{withdraw acc1 50} is a transaction, but \code{balSrc <- readTVar src} is an STM action. From the perspective
...
example. The examples that refer to the bank accounts (Section \ref{STMInterface}) use \code{acc1} and 
\code{acc2}. If \code{t1} or \code{t2} are used to denote transactions, this is always explicitly 
noted in the text. \code{a} and \code{b} denote pure values and are always introduced in the code.
