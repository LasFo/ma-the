...
\end{lstlisting}
This indeed solves the problem regarding the deadlock. In return we lose consistency. For a brief moment an we see an inconsistent state. 
...
most modern this is systems not acceptable, thus this solution is not feasible.
...
to retain the ACI properties. The failed validation indicates that transaction has read a value that is now outdated and possibly worked 
...
comitting the transaction. This implies the transaction may work on an inconsistent state until it attempts to commit. This means the transaction 
...
This allows both transactions to commit without rolling back either of them, if \code{t1} commit before \code{t2} (and \code{t1}
does not write any TVar that \code{t2} has accessed). If \code{t2} commit first, the conflict is detected when \code{t1} commits
and resolved by rolling back \code{t1}.
...
is ready the scheduler is allowed to swap this thread to \code{running} which grants the thread access to
the CPU. If the thread executes specific operation (for example the semaphore operation \code{P}) it enters 
the state suspended. Otherwise it is up to the scheduler to swap the thread to the \code{ready} state and
...
\code{>>}. In the text that explain the examples, we refer to \code{>>=} instead of \code{<-}.
