...
conflict are not detected until both, \code{t1} and \code{t2}, try to commit. If only \code{t1} commits, it does not 
...
withdraw src am = do
  ...
  putMVar src (balSrc - am)
\end{lstlisting}
\code{withdraw acc1 50} is a transaction, but \code{balSrc <- takeMVar src} is an STM action. From the perspective
...
examples. The examples that refer to the bank accounts (Section \ref{STMInterface}) use \code{acc1} and 
