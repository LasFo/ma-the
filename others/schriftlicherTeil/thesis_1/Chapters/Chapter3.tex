...
in \ref{sec:IFFun}. \code{MVar [MVar ()]} is the queue of the MVar that is needed to observe this TVar. 
Remember that this is needed to delay the rollback when \code{retry}
...
the \code{newValue}, \code{actualMVar}, and the explicit lock. \code{actualMVar} is the value holding 
...
\code{notifies} holds an IO action that is process when the transaction successfully commits.
...
TVar. To avoid this the IO-read is not executed directly. The function \code{buildVal} helps us to achieve this.
...
the modifications of \code{notifies}. A successful commit of the transaction means a 
...
new value. If the second field is \textbf{not} \code{Nothing}, it indicates that this value is modified 
...
\code{retryMVar} from the queues (if that has not already happend) and restarts.
...
stored in the \code{writeSet}, since \code{writeTVar} as well as \code{readTVar} extend 
the \code{writeSet}.
...
a \code{StmState} and returns an \code{StmResult}. Thus the function basically
...
Since we use a \code{case} expression to branch on the different \code{StmResults}, we 
could also extract the value from the \code{Maybe} type with pattern matching instead 
...
The implementation of \code{retry} and \code{orElse} are very simple. \code{retry} is a 
