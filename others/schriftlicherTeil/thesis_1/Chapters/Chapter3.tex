...
a \code{StmState} and returns an \code{StmResult}. Thus the function \code{>>=} basically
...
one (_,_) = True
\end{lstlisting}
We first create a TVar that hold a pair. In the second transaction we pattern match on this pair with the function \code{one}.
The value of the pair are not used in \code{one} and the pattern matching cannot fail. Nevertheless, the evaluation of \code{p}
